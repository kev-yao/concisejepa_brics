...
class Concise(nn.Module):
    ...
    def __init__(
        ...
        self.d_encoder = DrugEncoder(drug_layers, ligand_dim, drug_dim, activation=activation_choices[activation])
    ...
    def score(self, r_emb, b_size=32, cutoff=0.5):
        ...
        with torch.no_grad():
            for chunk in combinations.split(b_size):
                ...
                # Use cached forward
                res = self.forward_cached(chunk, r_emb_proj, None, is_morgan_fingerprint=False)
                scores.append(res["binding"])
